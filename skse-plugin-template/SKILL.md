...
Do not require extra tasks or repeated authorization for this process. When
the user has already asked for implementation, complete the explicitly
...
Check that the generator produces a minimal complete project, that arguments
land correctly in CMake/metadata, that existing files are never overwritten,
and that no network access happens by default. After actually configuring and
compiling the generated plugin, choose tests according to the newly added
features. Do not run, or claim, verification for modules that were not
introduced.
