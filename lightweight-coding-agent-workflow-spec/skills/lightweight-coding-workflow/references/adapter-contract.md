...
| `EXPERIMENTAL` | A runnable mapping exists but verification is incomplete or limited. | No by default in protocol 0.3. |
...
Only `VERIFIED` adapters can be selected by this workflow. A host does not become supported merely because its concepts resemble another host or because an authoring template was copied.
...
- the only matching adapter is `EXPERIMENTAL`, `AUTHORING_ONLY`, or `UNSUPPORTED`;
