...
本 skill 是 workflow 内置且自包含的 Python 工程规范。不要读取、触发或依赖环境中的其他 Python 规范 skill；与已通过当前 Harness 原生 loader 加载的内置 `small-project-code-contract` 同时使用。既有项目先服从 `pyproject.toml`、锁文件、格式化与检查配置、CI 和相邻第一方代码；只在触及范围内应用新默认值。
